...
#  Description: sets of recursion problems taken from CodeBat to complete
...
# is to consider the part of the array idxing at index idx and
# continuing to the end of the array. The caller can specify the
# whole array simply by passing idx as 0. No loops are needed --
# the recursive calls progress down the array.
def groupSum(idx, nums, target):
    if idx >= len(nums):
        return (target == 0)
    else:
        return groupSum(idx + 1, nums, target - nums[idx]) or groupSum(idx + 1, nums, target)



# Given an array of ints, is it possible to choose a group of some
# of the ints, beginning at the idx index, such that the group
# sums to the given target? However, with the additional constraint
# that all 6's must be chosen. (No loops needed.)
def groupSum6(idx, nums, target):
    if idx >= len(nums):
        return (target == 0)
    elif nums[idx] == 6:
        return groupSum6(idx + 1, nums, target - nums[idx])
    else:
        return groupSum6(idx + 1, nums, target - nums[idx]) or groupSum6(idx + 1, nums, target)
...
def groupNoAdj(idx, nums, target):
    if (idx >= len(nums)):
        return (target == 0)
    else:
        return groupNoAdj (idx + 2, nums, target - nums[idx]) or groupNoAdj(idx + 1, nums, target)
...
def groupSum5(idx, nums, target):
    if (idx >= len(nums)):
        return (target == 0)
    elif (nums[idx] % 5 == 0):
        if (idx < (len(nums) - 1) and nums[idx+1] == 1):
            return groupSum5(idx + 2, nums, target - nums[idx])
        return groupSum5(idx + 1, nums, target - nums[idx])
    else:
        return groupSum5(idx + 1, nums, target - nums[idx]) or groupSum5(idx + 1, nums, target)
...
def groupSumClump(idx, nums, target):
    if (idx >= len(nums)):
        return (target == 0)
    nextNum = idx + 1
    for i in range(len(nums)):
        if nextNum < i and nums[nextNum] == nums[idx]:
            nextNum += 1
    else:
        return groupSumClump(nextNum, nums, target - ((nextNum - idx) * nums[idx])) or groupSumClump(nextNum, nums, target)
...
def splitArrayHelper(nums, iterator, checker):
    if (iterator >= len(nums)):
        return (checker == 0)
    else:
        return splitArrayHelper(nums, iterator + 1, checker + nums[iterator]) or splitArrayHelper(nums, iterator + 1, checker - nums[iterator])
...
def splitOdd10Helper(nums, iterator, g1, g2):
    if (iterator >= len(nums)):
        return (g2 % 10 == 0 and g1 % 2 == 1 or g1 % 10 == 0 and g2 % 2 == 1)
    else:
        return splitOdd10Helper(nums, iterator + 1, g1 + nums[iterator], g2) or splitOdd10Helper(nums, iterator + 1, g1, g2 + nums[iterator])
...
def split53Helper(start, nums, by5, by3):

    if (start >= len(nums)):
        return (by5 == by3)
    elif (nums[start] % 5 == 0):
        return split53Helper(start + 1, nums, by5 + nums[start], by3)
    elif (nums[start] % 3 == 0):
        return split53Helper(start + 1, nums, by5, by3 + nums[start])
    else:
        return split53Helper(start + 1, nums, by5 + nums[start], by3) or \
               split53Helper(start + 1, nums, by5, by3 + nums[start])
